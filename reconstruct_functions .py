...
def reconstruct_dataset(size,directory,output_file,mode,seqID="0",region="0",update=False,filemode="w"):
	...
	#If the user is interested in sequence of interest
		...
		#Check if the user is interested in a region in particular
			...
			try:
				...
				ToReconstruct=get_files(Dir=directory+SeqDir,Seq=seqID,RegionStart=region_start,RegionStop=region_stop)
				#If variants
				if(mode=="Variants"):
	...
	#Otherwise this is a normal reconstruction of annotation/variants/alignment dataset
		...
		#NEW, check if the check.txt file is generated from parse_functions.py
		if(os.path.isfile(directory+"/converted.txt")):
			#Convert SAM back to BAM and remove the SAM file
			print("Now converting SAM 2 BAM")
			Popen("samtools view -S -b -h -o ../Extracted_Alignment.bam" + " " + reconstruct.name, shell=True).wait()
...
def extract_dataset(dataset,seqID,region):
	...
	#Check if the user provided a seqID
		...
		#Inform the user
		if(region!="0"):
			print("Now reconstructing data contained in the "+dataset+" dataset for sequence "+seqID+" in the region "+region)
		else:
		...
		for subfile in file_list:
			...
			reconstruct_dataset(size=int(subfile[3]),directory=subfile[2],output_file="../Extracted_"+dataset+extension,mode=subfile[1],seqID=seqID,region=region,filemode="a")			
		#Inform the user
	...
	#Otherwise the user wants to reconstruct the entire dataset
		...
		if(region!="0"):
			...
			print("Now reconstructing data contained in the "+dataset+" dataset for sequence "+seqID)		#Delete the file if it already exists
		if(os.path.isfile("../Extracted_"+dataset+extension)):
			os.remove("../Extracted_"+dataset+extension)
		#Loop through the files contained in the dataset
		for subfile in file_list:
			#Reconstruct the subfile
			reconstruct_dataset(size=int(subfile[3]),directory=subfile[2],output_file="../Extracted_"+dataset+extension,mode=subfile[1],seqID="0",region="0",filemode="a")			
		#Inform the user
		print("Reconstruction completed.")
