...
for seqKey in genome_report.keys():
    # Print the seqID
    output_file.write("\n\t------\n\t|\n\t| Sequence: " +
                      genome_report[seqKey][0] + " (" + genome_report[seqKey][1] + " nt)")
    # Print the number of contigs, gaps and the percentage of Ns
    output_file.write("\n\t| Number of contigs: " + str(genome_report[seqKey][2]) + "\n\t| Number of gaps: " + str(
        genome_report[seqKey][2] - 1) + "\n\t| Percentage of unknown sequence: " +
        str(genome_report[seqKey][3] / (int(genome_report[seqKey][1]) / 100)) + "%")
    ...
    # If there are VCF files
        ...
        for vcf_file in genome_report[seqKey][4][0].keys():
            output_file.write("\n\t|\t-" + vcf_file + ": " +
                              str(genome_report[seqKey][4][0][vcf_file]))
    ...
    # If there are GFF files
        ...
        for gff_file in genome_report[seqKey][4][1].keys():
            output_file.write("\n\t|\t-" + gff_file + ": " +
                              str(genome_report[seqKey][4][1][gff_file]))
    ...
    # If there are SAM files
        ...
        for sam_file in genome_report[seqKey][4][2].keys():
            output_file.write("\n\t|\t-" + sam_file + ": " +
                              str(genome_report[seqKey][4][2][sam_file]))
...
with open("./RepoMap.txt", "r") as repomap:
    ...
    for line in repomap:
        ...
        if(line[1] == "Annotation"):
            ...
            variants_number += 1
repomap.close()
print("Number of sequences stored in the Genome dataset: " +
      str(len(genome_report.keys())))
...
print("Number of files contained in the Alignment dataset: " +
      str(alignment_number) + "\n")
