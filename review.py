...
Popen("git checkout "+str(sys.argv[2])+" 2> /dev/null", shell=True).wait()
reconstruct_dataset(size=60, directory="./Genome",
                    output_file="./.git/info/temporary_directory/assembly_2.fa", mode="Genome")
# Checkout back to the master
Popen("git checkout master 2> /dev/null", shell=True).wait()
...
# Otherwise it is required to perform the alignment between both assemblies
    ...
    variables = obtain_alignment(old_assembly="./.git/info/temporary_directory/assembly_1.fa", new_assembly="./.git/info/temporary_directory/assembly_2.fa",
                                 directory="./.git/info/temporary_directory",
                                 threads=number_threads, ToUpdate=ToUpdate, alignment_pickle=alignment_pickle, aligner_switch=aligner_switch,
...
if(merge):
    ...
    # Write info
    report_file.write("\nNow producing a merge sequences report between commits "+commit_A+" and "+commit_B +
                      ". Warning: If you have added any data into the repository but did not commit, these changes will be lost.\n")
    ...
    for newID in merge_dict.keys():
        ...
        if(len(merge_dict[newID]) > 1):
            ...
            report_file.write("\t------\n\t|\n\t| Sequence "+newID+" (" +
                              delta_dict[merge_dict[newID][0]][1]+" nt) is a result of "+str(len(merge_dict[newID]))+" merged sequences:")
...
# Otherwise produce a normal report
    ...
    for oldID in summary_Dict.keys():
        ...
        if(summary_Dict[oldID][0] == "identical"):
            table[oldID] = ["\t------\n\t|\n\t| Sequence: "+oldID +
                            " ("+summary_Dict[oldID][1]+" nt) --> "+OldNewID_Dict[oldID]+" ("+summary_Dict[oldID][1]+" nt) (identical sequences).\n\t|\n\t------"]
            identical_count += 1
        # If it is a deleted sequence
        elif(summary_Dict[oldID][0] == "deleted"):
            table[oldID] = ["\t------\n\t|\n\t| Sequence: "+oldID +
                            " ("+summary_Dict[oldID][1]+" nt) was deleted from the assemlby.\n\t|\n\t------"]
            deleted_count += 1
        # Otherwise it was a reversed sequence
        else:
            table[oldID] = ["\t------\n\t|\n\t| Sequence: "+oldID +
                            " ("+summary_Dict[oldID][1]+" nt) --> "+OldNewID_Dict[oldID]+" ("+summary_Dict[oldID][1]+" nt) (reversed sequences).\n\t|\n\t------"]
            reversed_count += 1
    ...
    for oldID in delta_dict.keys():
        ...
        for contig in coords_dict[oldID]:
            ...
            i += 1
            contig = contig.split("\t")
            table[oldID].append("\t|\t-Contig "+str(i)+": "+contig[0]+"-"+contig[1] +
                                " --> "+contig[2]+"-"+contig[3]+"\t ("+contig[6]+"% sim.)")
            # Sum coverage
