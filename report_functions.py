...
#Create a function to count lines of a given file
...
#Create a function to create a repo list about the reporsitory
def report_repo():
	...
	with open("./Genome/Map.txt","r") as map_file:
		...
		for line in map_file:
			...
			#Otherwise it is a sequence feature
				...
				if(line[0]=="N"):
					genome_report[seqKey][3]+=(int(line[1].split()[1])-int(line[1].split()[0]))
			...
			with open(subfile[1]+"/SeqIDs_Map.txt","r") as map_file:
				...
				for line in map_file:
					...
					if(line[0]!=">"):
						...
						line_count+=count_lines(subfile[1]+line.rstrip().split("\t")[0])
