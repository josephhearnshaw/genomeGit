...
inversions = int(sys.argv[9])


def inversion_bool(inversions):
    if(inversions == 0):
        inversion_statement = "No"
    else:
        inversion_statement = "Yes"
    return inversion_statement
...
def aligner_status(ToUpdate):
    ...
    inversion_statement = inversion_bool(inversions)
    ...
    if (ToUpdate == "empty"):
        ...
        if aligner_switch == 1:
            ...
            print("\n\n**Aligner chosen is MashMap + Nucmer (GenomeGit 2.1) with the following "
                  ...
                  "\n\n\t -Kmer: {}\n\n\t -Segment length: {}\n\n\t -Inversions looked for? {}\n\n"
                  .format(percent_identity, kmer, segLength, inversion_statement))
        else:
            aligner = "Nucmer"
            print("\n\n**Aligner chosen is Nucmer only (GenomeGit 2.0)**\n\n -Inversions looked for? {}".format(inversion_statement))
    ...
    if(ToUpdate["Genome"][0][2] == "1"):
        ...
        if(file_size == "1"):
            print("\n*** NOW STARTING UPDATE OF THE REPOSITORY: {} ({} MB) -->"
                  " {} (<1 MB)***\n".format(ToUpdate["Genome"][0][0], ToUpdate["Genome"][0][2], os.path.basename(new_assembly)))
        else:
            print("\n*** NOW STARTING UPDATE OF THE REPOSITORY: {} ({} MB) --->"
                  " {} ({} MB)***\n".format(ToUpdate["Genome"][0][0], ToUpdate["Genome"][0][2], os.path.basename(new_assembly), file_size))
...
def reconstruct_annotation_variants(ToUpdate):
    ...
    for dataset in ToUpdate.keys():
        ...
        if(dataset != "Genome" and len(ToUpdate[dataset]) != 0):
            ...
            for subfile in ToUpdate[dataset]:
                reconstruct_dataset(size=1, directory=subfile[1], output_file="./temporary_directory/{}".format(subfile[0]),
                ...
                if(dataset == "Annotation" or dataset == "Alignment"):
                    # Get all the headers (starts wiht #) for the alignment files, and all non header data, sort column 1 and column 2 (numerically) sorted
                    # Then compress and create indexes and use tabix to create the library
                    Popen("(grep ""^#"" ./temporary_directory/{}_A; grep -v ""^#"" ./temporary_directory/{}_A | sort -V -k1,1 -k2,2n)\
					| bgzip > ./temporary_directory/{}_A.gz; tabix -p vcf ./temporary_directory/{}_A.gz;".format(subfile[0], subfile[0],
                                                                                                  subfile[0], subfile[0]), shell=True).wait()
                    # Do this for dataset B too
                    Popen("(grep ""^#"" ./temporary_directory/{}_B; grep -v ""^#"" ./temporary_directory/{}_B | sort -V -k1,1 -k2,2n)\
					 | bgzip > ./temporary_directory/{}_B.gz; tabix -p vcf ./temporary_directory/{}_B.gz;".format(subfile[0], subfile[0],
                                                                                                   subfile[0], subfile[0]), shell=True).wait()
                ...
                # Otherwise there was an error
                    ...
                    print("***INTERNAL ERROR*** DATASET NOT RECOGNIZED: {}".format(dataset))
...
def obtain_variables(alignment_pickle):
    ...
    It will return the varaibles obtained, including the new alignment if performed.
    ...
    if(os.path.isdir(alignment_pickle)):
        ...
        variables = load_variables("{}/pickle".format(alignment_pickle))
        tabix_queries, OldNewID_Dict, alignment_pickle, summary_Dict, file_crack = variables
    # Otherwise it is necessary to obtain the alignment
        ...
        variables = obtain_alignment(old_assembly="./temporary_directory/genome_old.fa",
                                     new_assembly=new_assembly, directory="./temporary_directory", threads=number_threads,
                                     ToUpdate=ToUpdate, alignment_pickle=alignment_pickle, aligner_switch=aligner_switch, percent_identity=percent_identity,
                                     kmer=kmer, segLength=segLength, inversions=inversions)
...
reconstruct_dataset(size=60, directory="./Genome",
                    output_file="./temporary_directory/genome_old.fa", mode="Genome", seqID="0", region="0")
...
interpret_alignment(queries=tabix_queries, oldnew=OldNewID_Dict,
                    threads=number_threads, ToUpdate=ToUpdate, tlength=template_length, filecrack=file_crack)
