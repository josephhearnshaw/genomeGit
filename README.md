...
Flags specific to Nucmer4 (```--c=<x>``` or ```--mincluster=<x>```) can be used ([see the NUCmer documentation for information regarding these flags](http://mummer.sourceforge.net/manual/#nucmer)). Likewise, the flags ```--k=<x>``` or ```--kmer=<x>``` and ```--s=<x>``` or ```--segLength=<x>``` and ```--pi=<x>``` can be used in MashMap2 ([see the MashMap GitHub page for information regarding these flags](https://github.com/marbl/MashMap)). In addition, the user can use the flag ```--ms=<x>``` as either 1 or 2, where 2 will also detect merges (but not splits). By default, it'll detect splits (```--ms=1```), whereas Nucmer4 only will detect splits and merges. 
