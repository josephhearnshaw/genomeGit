...
def reconstruct_dataset(size, directory, output_file, mode, seqID="0", region="0", update=False, filemode="w"):
    ...
    # Otherwise this is a normal reconstruction of annotation/variants/alignment dataset
        ...
        comments = open("{}/Comments.txt".format(directory), "r")
        # Only append comments if filemode is different than append (no comments if the user reconstruct a region)
        # NEW, reversed the if check, it should add comments if the entire file reconstructed
        if('.bam' in output_file or '.sam' in output_file):
            if(filemode == "a"):
                for line in comments:
                    reconstruct.write(line)
            # Open the seqID map
            seqID_map = open("{}/SeqIDs_Map.txt".format(directory), "r")
            # Loop through the map
            for line in seqID_map:
                # If the line starts with >, it is a new seqID. Ignore it
                if(line[0] != ">"):
        ...
        if(os.path.isfile("{}/converted.txt".format(directory))):
